...
### 🧹 Bug Fixes
- Fixed an issue with the `use-image-gallery` hook where the `imageCodes` prop was not being correctly split into an array.
- Updated the `GoalImageGallery` component to ensure the `currentCompany` prop can accept both string and number types for better flexibility.
- Removed unused `clipboardSupport` prop from the `GoalImageGallery` component to avoid confusion.
- Ensured consistent handling of `currentCompany` across all components.
